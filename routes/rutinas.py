...
def crear():
    ...
    try:
        ...
        cursor.execute('''INSERT INTO rutinas (id_usuario, nombre) values (%s,%s)''',
                       (current_user, nombre))
        
        # Guardamos el nuevo registro
        cursor.connection.commit()

        return jsonify({"mensaje":"La rutina se creo correctamente"}), 201
...
def get():
    ...
    query = '''
               SELECT a.id_usuario, a.nombre, b.nombre, b.email
               FROM rutinas as a 
...
def actualizar(id_rutina):
    ...
    try:
        # Hacemos el update
        cursor.execute("UPDATE rutinas SET nombre = %s WHERE id_rutina = %s", 
                       (nombre, id_rutina))
        cursor.connection.commit()
